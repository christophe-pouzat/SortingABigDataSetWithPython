...
plt.subplot(211)
Evts[Evts.good_events(valley=True,upper_thr=10,lower_thr=-6),:][:500,:].plot()
plt.subplot(212)
Evts[-Evts.good_events(valley=True,upper_thr=10,lower_thr=-6),:].plot()
#+END_SRC

#+RESULTS: Sample-good_events-example
: <matplotlib.axes._subplots.AxesSubplot object at 0x7f01d6730438>
: The MAD is displayed with an offset of: 2059.0
: <matplotlib.axes._subplots.AxesSubplot object at 0x7f01d576bc18>
: The MAD is displayed with an offset of: 2087.0
...
class Sample4Clust(Sample):
    ...
    def catalogue(self,before=49,after=50):
        ...
        target_median = np.repeat(self._DS._MCD.median,2)
        ...
        for i in range(np.max(self.cluster)+1):
            center = samp[self.cluster==i,:].median
            error = samp[self.cluster==i,:].mad[critical_points]/np.sqrt(sum(self.cluster==i))
            if np.all(np.abs(center[critical_points]-target_median)/error < 3):
...
#+BEGIN_SRC python :session *Python* :results output :exports both
EvtsG = Sample4Clust(Evts,valley=True,upper_thr=10,lower_thr=-6)
print(EvtsG)
#+END_SRC
...
:   The instance contains: 1988 events.
:   The cuts are: 32 sampling points long.
:   The reference time is located at position 8 within each cut (indexes start at 0).
...
We could draw on the same graph all the events of cluster 0 (in black) and all the events of cluster 2 (in orange) with:

#+NAME: Sample4Clust-plot-example3
#+BEGIN_SRC python :session *Python* :results silent
EvtsG[EvtsG.cluster==0].plot(show_median=False,show_mad=False)
EvtsG[EvtsG.cluster==2].plot(events_color='orange',show_median=False,show_mad=False)
plt.ylim([-10,10])
#+END_SRC

We could merge these two clusters with:

#+NAME: Sample4Clust-merge_clusters-example
#+BEGIN_SRC python :session *Python* :results silent
EvtsG.merge_clusters([0,2])
#+END_SRC

To see in what cluster they ended up we call the =print= special method and compare its output with our initial call to the =find_clusters= method:
...
We see that the new cluster 0 has the same number of events than the sum of the events of two previous clusters we just merged (0 and 2).

We can split this new cluster 0 into 2 clusters using the kmeans with:

#+NAME: Sample4Clust-split_cluster-example
#+BEGIN_SRC python :session *Python* :results output :exports both
EvtsG.split_cluster(0,2)
#+END_SRC

#+RESULTS: Sample4Clust-split_cluster-example
: Number of events of each sub-cluster (percent of events) of cluster 0:
: Sub-cluster  0: 68  (31.6%)
: Sub-cluster  1: 147  (68.4%)
: Total number of events: 215

And the =print= special method gives:
...
We see that the first two clusters are not back to baseline so we take more points after the reference time with:

#+NAME: Sample4Clust-catalogue-example2
#+BEGIN_SRC python :session *Python* :results output :exports both
center_catalogue = EvtsG.catalogue(after=70)
#+END_SRC
