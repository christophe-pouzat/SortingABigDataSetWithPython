...
class MultiChannelData:
    ...
    def plot(self,linewidth=0.2,color='black'):
        ...
        plt.xlabel("Time (s)")
    def derivative(self):
        """Return a MultiChannelData instance with the time derivative of the original one."""
        from scipy.signal import fftconvolve
        from numpy import apply_along_axis as apply
        return MultiChannelData(apply(lambda x: fftconvolve(x,np.array([1,0,-1])/2.,'same'),1, self._data),
                                self._start,self._sampling_rate)

#+END_SRC

** DetectedSpikes Class

=Class= =DetectedSpikes= will store the spikes (local maxima or minima) detection results.

#+NAME: DetectedSpikes-Class-definition
#+BEGIN_SRC python :session *Python* :results silent
class DetectedSpikes:
    """Detected spikes."""
    def __init__(self,MCD_instance,threshold=4,filter_length=3,channels=None,minimal_dist=15,not_zero=1e-3):
        ...
        channels        a list with the channels to use for detection
        ...
        if channels is None:
            channels = range(MCD_instance._data.shape[0])
        ...
        self._minimal_dist = minimal_dist
        self._not_zero = not_zero
        data = MCD_instance.get_data().copy()[channels,:]
        data_median = np.median(data,axis=1)
        data = data-data_median.reshape(len(channels),1)
        data_mad = np.median(np.abs(data),axis=1)*1.4826
        data = data / data_mad.reshape(len(channels),1)
        data_filtered = apply(lambda x:
                              fftconvolve(x,np.ones(filter_length)/filter_length,'same'),
                              1,data)
        data_filtered_median = np.median(data_filtered,axis=1)
        data_filtered = data_filtered-data_filtered_median.reshape(len(channels),1)
        data_filtered_mad = np.median(np.abs(data_filtered),axis=1)*1.4826
        data_filtered = data_filtered / data_filtered_mad.reshape(len(channels),1)
        if threshold < 0:
            data_filtered[data_filtered > threshold] = 0
            data_filtered = -data_filtered.sum(0)
        else:
            data_filtered[data_filtered < threshold] = 0
            data_filtered = data_filtered.sum(0)
        dx = fftconvolve(data_filtered,np.array([1,0,-1])/2.,'same') 
        dx[np.abs(dx) < not_zero] = 0
        dx = np.diff(np.sign(dx))
        pos = np.arange(len(dx))[dx < 0]
        self._spike_positions = pos[:-1][np.diff(pos) > minimal_dist]
        self._k = -1
    def __len__(self):
        """Return the number of detected spikes."""
        return len(self._spike_positions)
    def __str__(self):
        """Controls the printed version of the instance."""
        import numpy as np
        return "An instance of DetectedSpikes.\n" \
            + "  Spikes were detected on " + str(len(self._channels)) + " channels simultaneously (" \
            + str(list(self._channels)) + ").\n" \
            + "  A boxe filter with length " + str(self._filter_length) + " was used.\n" \
            + "  The detection threshold (multiple of the MAD) was set at " + str(self._threshold) + ".\n" \
            + "  " + str(len(self._spike_positions)) + " putative spikes were detected. The smallest inter spike interval is: " \
            + str(np.min(np.diff(self._spike_positions))) + ", and the largest is: " + str(np.max(np.diff(self._spike_positions))) \
            + ".\n  The mean ISI is: " + str(int(np.mean(np.diff(self._spike_positions)))) + " and the ISI SD is: " \
            + str(int(np.std(np.diff(self._spike_positions)))) + ".\n"
    def __next__(self):
        """Return the next element, or else raise StopIteration error."""
        self._k += 1
        if self._k < len(self._spike_positions):
            return self._spike_positions[self._k]
        else:
            raise StopIteration()
    def __iter__(self):
    ...
    def get_spike_positions(self):
        """Return an array with the spike positions."""
        return self._spike_positions
    def plot(self,linewidth=0.2,color='black'):
        """Plot the data with the detected spikes."""
        import matplotlib.pyplot as plt
        import numpy as np
        nb_chan = self._MCD.nb_channels()
        data_min = np.min(self._MCD._data,axis=1) 
        data_max = np.max(self._MCD._data,axis=1)
        display_offset = list(np.cumsum(np.array([0] +
                                                 [data_max[i]-data_min[i-1]
                                                  for i in range(1,nb_chan)])))
        tt = np.arange(len(self._MCD))/self._MCD._sampling_rate+self._MCD._start
        for i in range(nb_chan):
            plt.plot(tt,self._MCD._data[i,:]-display_offset[i],
                     linewidth=linewidth,color=color)
            plt.plot(tt[self._spike_positions],
                     self._MCD._data[i,self._spike_positions]-display_offset[i],'ro')
        plt.yticks([])
        plt.xlim([self._MCD._start,self._MCD._start+self._MCD.duration()])
        plt.xlabel("Time (s)")
...
class Sample:
    ...
    def __init__(self, DS_instance, before=14, after=30):
        ...
        ns,dl = DS_instance._MCD._data.shape
        ...
        def cut_sgl_evt(evt_pos):
            ...
            kw = keep[within]
            cut[:,within] = DS_instance._MCD._data[:,kw].copy()
            return cut.reshape(cs)
        res = np.zeros((len(DS_instance),(before+after+1)*ns))
        for i,p in enumerate(DS_instance.get_spike_positions()):
            res[i,:] = cut_sgl_evt(p)
        self._data = res
        self._before = before
        self._after = after
    ...
    def mad(self):
        """Sample's MAD."""
        import numpy as np
        return np.median(np.abs(self._data - self.median().reshape(1,self.shape()[1])),axis=0)*1.4826
    def std(self):
        """Sample's STD."""
        import numpy as np
    ...
    def plot(self,events_color='black',events_lw=0.1,
        ...
        cut_length = self._before + self._after + 1
        for i in range(self.shape()[0]):
            plt.plot(self.get_data()[i,:], color=events_color, lw=events_lw)
        if show_median:
            MEDIAN = self.median()
        ...
        plt.xticks([])
        plt.ylim([np.min(self.get_data()),np.max(self.get_data())])
