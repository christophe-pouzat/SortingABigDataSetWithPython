...
* Defining classes and methods for a systematic analysis of the remaining of the experiment
...
class MultiChannelData:
    ...
    def sample2time(self,sample_point):
        ...
        if not 0 <= sample_point <= self._length:
            raise ValueError('sample_point must non negative  positive and smaller than ' + str(self._length))
        return sample_point/self._sampling_rate + self._start
    def time2sample(self,time):
        """Converts time in second to sample point."""
        if not self._start <= time < self._start+duration(self):
            raise ValueError('time must be larger than ' + str(start) + ' and  smaller than ' + str(self._start+duration(self)))
    ...
    def quantiles(self, prob=[0,0.25,0.5,0.75,1]):
        ...
        """
        from scipy.stats.mstats import mquantiles
        return mquantiles(self._data,prob, axis=1)
