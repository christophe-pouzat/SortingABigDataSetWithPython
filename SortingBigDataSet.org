...
: <matplotlib.axes._subplots.AxesSubplot object at 0x7f22465c75c0>
: The MAD is displayed with an offset of: 2061.0
: <matplotlib.axes._subplots.AxesSubplot object at 0x7f222a08afd0>
...
class Sample4Clust(Sample):
    ...
    def project(self,components=slice(4)):
        """Project events onto principal components subspace specified by components."""
        import numpy as np
...
We can split this new cluster 1 into 2 clusters using the kmeans with:
...
We see that former cluster 1 became clusters 1 and 2. If we look at those two we see that the split was not done as we wanted so we merge the two clusters first:
...
Here it's a bit tricky. We want to pull out, among the events of what is now cluster 1, the ones with a pronounced early peak *on channel 1*. If we look at our PCA plot, we see that PCs 0, 2 and 3 (but not 1) catch variability mainly on channel 1. We will therefore use projections on those three PCs. Then trying to split cluster 1 in two clusters using a Gaussian mixture model fitted with the EM algorithm still does not do exactly the job, so we split in 3 first and merge afterwards the two similar ones:

#+NAME: Sample4Clust-split_cluster-example2
#+BEGIN_SRC python :session *Python* :results output :exports both
EvtsG.split_cluster(cluster2split=1,split_into=3,method='EM4GMM',components=[0,2,3])
#+END_SRC

#+RESULTS: Sample4Clust-split_cluster-example2
: Number of events of each sub-cluster (percent of events) of cluster 1:
: Sub-cluster  0: 150  (53.4%)
: Sub-cluster  1: 93  (33.1%)
: Sub-cluster  2: 38  (13.5%)
: Total number of events: 281
...
We see that former cluster 1 has be split into new clusters 0, 2 and 3. A plot of those:
...
We can draw on the same graph all the events of cluster 2 (in black) and all the events of cluster 3 (in orange) with:

#+NAME: Sample4Clust-plot-example3
#+BEGIN_SRC python :session *Python* :results silent
EvtsG[EvtsG.cluster==2].plot(show_median=False,show_mad=False)
EvtsG[EvtsG.cluster==3].plot(events_color='orange',show_median=False,show_mad=False)
plt.ylim([-15,10])
#+END_SRC

Since they are indeed very similar, we merge them:

#+NAME: Sample4Clust-merge_clusters-example2
#+BEGIN_SRC python :session *Python* :results output :exports both
EvtsG.merge_clusters([2,3])
print(EvtsG)
#+END_SRC

#+RESULTS: Sample4Clust-merge_clusters-example2
#+begin_example
...
class Sorted:
    ...
    def __init__(self,MCD_instance,catalogue,sorting):
        ...
        sorting       an N x 3 integer array where N is the number of spikes
                      ...
                      the closest and the 'actual' position.
        """
        import numpy as np
        if not isinstance(MCD_instance, MultiChannelData):
            raise TypeError('MCD_instance must be a MultiChannelData instance')
    ...
    def predict(self):
        ...
        ref_idx = np.arange(-self.catalogue['before'],self.catalogue['after'])
...
#+BEGIN_SRC python :session *Python* :results silent
input0 = np.column_stack((EvtsG.cluster,EvtsG._DS.spike_positions,np.zeros(len(EvtsG.cluster),dtype=np.int)))
Sorting0 = Sorted(Data,center_catalogue,input0)
#+END_SRC

#+BEGIN_SRC python :session *Python* :results silent
toto = Sorting0.predict
#+END_SRC
