...
#+CAPTION: Robust estimates of the central event (black) and of the sample's dispersion around the central event (red) obtained with "long" (100 sampling points) cuts. We see clearly that the dispersion is back to noise level 10 points before the peak and 30 points after the peak. We also see that =data[3]= brings very little information.
...
We can check how many events got unclassified on a total of src_python[:results pp :session *Python*]{len(sp1)} =583=:
...
pred2 = swp.predict_data(round2,centers,data2.shape[0],data2.shape[1])
data3 = data2 - pred2
...
pred3 = swp.predict_data(round3,centers,data3.shape[0],data3.shape[1])
data4 = data3 - pred3
