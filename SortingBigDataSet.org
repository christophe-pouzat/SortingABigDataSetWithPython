...
A length (=len=) special method is defined and subsetting can be performed on the object like on a "classical" matrix.  
...
We can then get the five-number summary with:
