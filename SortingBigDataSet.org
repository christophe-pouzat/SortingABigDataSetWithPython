...
class MultiChannelData:
    ...
    def __getitem__(self,key):
        ...
        if isinstance(key[0],slice) or isinstance(key[0],int):
            channel_names = self.channel_names[key[0]]
        elif isinstance(key[0],str):
            channel_names = [key[0]]
        elif all([n in self.channel_names for n in key[0]]):
            channel_names = key[0]
        else:
            channel_names = [self.channel_names[n] for n in key[0]]
...
+ =threshold=: the detection threshold used as a multiple of the MAD on each site. If positive respectively negative, peaks respectively valley are detected.
...
:   A boxe filter with length 5 was used.
